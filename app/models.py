...
class Teacher(AbstractUser, BaseModel):
...
class Test(BaseModel):
    ...
    created_by = models.ForeignKey(Teacher, on_delete=models.SET_NULL, null=True, related_name='tests',
...
class TopicQuestion(BaseModel):
    """Непосредственно вопрос с заданием, который может попасться студенту"""
    ...
    def __str__(self):
        answer_type = [ans.value for ans in AnswerTypeChoices if self.answer_type == ans.name][0]
...
class StudentTest(BaseModel):
    ...
    def __str__(self):
        return f'{self.student_name} {self.group} {self.test}'

    class Meta:
        verbose_name = 'Тест студента'
        verbose_name_plural = 'Тесты студентов'
        unique_together = ('student_name', 'test')
...
class StudentAnswer(BaseModel):
    """Ответ студента в конкретном тесте на конкретный вопрос"""
